...
START_TEST (test_alter_column_change_name) {
	...
	ck_assert(strcmp(table->format->fields[1]->name, "HEIGHT") == 0);
	ck_assert(strcmp(table->format->fields[1]->type, "INT") == 0);
...
START_TEST (test_alter_column_add_column) {
	...
	ck_assert(table->format->number_of_fields == 5);
	ck_assert(strcmp(table->format->fields[4]->name, "NEW_COLUMN") == 0);
	ck_assert(strcmp(table->format->fields[4]->type, "INT") == 0);
} END_TEST
